...
describe("Issuing per card type redemptions", () => {
  requiresDatabase();
  const now = new Date();
  ...
  it("expect a transaction to fail if the number of redemptions is over the redemption limit", async () => {
    ...
    await createRebateCriteria({
      ...
      redemptionLimit: 1,
      descriptionForTesting: 'Per Card Redemption',
      redemptionLimitType: RedemptionLimitType.PerCard,
    ...
    expect(failedTransaction.status).to.eq(TransactionStatus.ClientReachedCardRedLimit);
  });
